...
class TemplateAI : public AscentScriptCreatureAI
{
	ASCENTSCRIPT_FACTORY_FUNCTION(TemplateAI, AscentScriptCreatureAI);
	TemplateAI(Creature* pCreature) : AscentScriptCreatureAI(pCreature)
	{
	}
};
...
class MutantWarHoundAI : public AscentScriptCreatureAI
{
	...
	void OnDied(Unit* pKiller)
	{
		...
		Aura* pAura = NULL;
		pAura = new Aura(dbcSpell.LookupEntry(MUTANT_WAR_HOUND_CLOUD_OF_DISEASE),(uint32)20000, _unit, _unit);
		if (pAura != NULL)
		{
			_unit->AddAura(pAura);
		}
...
public:
	...
	void AIUpdate()
	{
		...
		if (t > AuraChange)
		{
			...
			Aura* aura = NULL;
			aura = new Aura(dbcSpell.LookupEntry(SpellId),(uint32)15000, _unit, _unit);
			if (aura)
			{
				_unit->AddAura(aura);
			}
